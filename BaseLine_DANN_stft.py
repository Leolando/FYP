...
def load(batch_size, kwargs):
    ...
    # print(target_stft_set[0])
    # print(target_stft_set[0].shape)
    # img=target_stft_set[0].reshape(65,33)*125
    # pil_img = Image.fromarray(np.uint8(img))
    # pil_img.show()

    # dataCom = source_data[0]
    # TimeFrequencyCWT(dataCom, fs=2000, totalscal=10, wavelet='cgau8')

    # t = np.linspace(-100, 100, 200, endpoint=False)
    # sig = target_data[0]
    # print(sig.shape)
    # widths = np.arange(1, 100)
    # cwtmatr = signal.cwt(sig, signal.ricker, widths)
    # print(cwtmatr.shape)
    # plt.imshow(cwtmatr, extent=[-100, 100, 1, 51], cmap='PRGn', aspect='auto',
...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self):
        ...
        # self.conv1 = nn.Sequential(
        #     nn.Conv2d(1, 30, kernel_size=5, stride=1),  # 61, 29
        #     nn.BatchNorm2d(30),
        #     nn.MaxPool2d(2),  # 30,14
        #     nn.ReLU()
        # )
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 30, kernel_size=3, stride=1),  # 61, 29
            nn.BatchNorm2d(30),
            nn.MaxPool2d(2),  # 30,14
            nn.ReLU()
        )
        ...
        # )
        self.conv2 = nn.Sequential(
            nn.Conv2d(30, 60, kernel_size=3, stride=1),  # 26, 10
            nn.BatchNorm2d(60),
            nn.MaxPool2d(2),  # 13*5
            nn.ReLU()
        )

        self.residualBlock = nn.Sequential(Residual(50,64, use_1x1conv=True),
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(1680, 128),  # 30*63 = 1890
            nn.BatchNorm1d(128),
            nn.ReLU()
        )
        self.dpout = nn.Dropout(0.4)
...
def train_epoch(source_dataloader, target_dataloader, current):
    ...
    for i, ((source_data, source_label), (target_data, _)) in enumerate(zip(source_dataloader, target_dataloader)):
        ...
        loss =0.01*class_criterion(class_logits, source_label) - lamb* domain_criterion(domain_logits, domain_label)
...
6: 1+mmd
7: 6+2
8:1+正常训练s/t识别器时，暂停(s)源域特征提取器的迭代跟新
'''
